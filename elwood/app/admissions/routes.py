...
@admin_required
def enquiries():
    all_enquiries = Enquiry.query.filter_by(school_id=current_user.school_id).order_by(Enquiry.created_at.desc()).all()
    return render_template('admissions/enquiries.html', enquiries=all_enquiries)

@bp.route('/applications')
@admin_required
def applications():
    apps = AdmissionApplication.query.filter_by(school_id=current_user.school_id).order_by(AdmissionApplication.submitted_at.desc()).all()
    return render_template('admissions/applications.html', applications=apps)

@bp.route('/public/<int:school_id>/inquire', methods=['GET', 'POST'])
def public_inquire(school_id):
    ...
    if request.method == 'POST':
        ...
        new_enq = Enquiry(
            school_id=school_id, student_name=student_name, parent_name=parent_name,
        ...
        return redirect(url_for('admissions.public_inquire', school_id=school_id))
        
    return render_template('admissions/public_inquire.html', school=school)

@bp.route('/enquiries/<int:eid>/status', methods=['POST'])
@admin_required
def update_enquiry_status(eid):
    enq = Enquiry.query.get_or_404(eid)
    if enq.school_id != current_user.school_id:
