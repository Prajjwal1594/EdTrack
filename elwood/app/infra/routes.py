...
def library():
    ...
    books = LibraryBook.query.filter_by(school_id=current_user.school_id).all()
    issues = BookIssue.query.filter_by(school_id=current_user.school_id, status='Issued').all()
...
def transport():
    if request.method == 'POST':
        ...
        new_route = TransportRoute(
            school_id=current_user.school_id, route_name=route_name,
    ...
    routes = TransportRoute.query.filter_by(school_id=current_user.school_id).all()
...
def allocate_transport():
    ...
    if existing:
        ...
        new_alloc = TransportAllocation(
            ...
            school_id=current_user.school_id, pickup_point=pickup_point
        )
...
def hostel():
    if request.method == 'POST':
        ...
        new_room = HostelRoom(
            school_id=current_user.school_id, hostel_name=hostel_name,
    ...
    rooms = HostelRoom.query.filter_by(school_id=current_user.school_id).all()
    students = Student.query.join(User).filter(User.school_id == current_user.school_id).all()
...
def delete_hostel(id):
    ...
    if room.school_id != current_user.school_id:
        flash('Access denied.', 'danger')
        return redirect(url_for('infra.hostel'))
...
def allocate_hostel():
    ...
    if existing:
        ...
        new_alloc = HostelAllocation(
            ...
            school_id=current_user.school_id, status='Occupied'
        )
        db.session.add(new_alloc)
...
def inventory():
    if request.method == 'POST':
        ...
        if action == 'add_category':
            ...
            new_cat = InventoryCategory(school_id=current_user.school_id, name=cat_name)
    ...
    items = InventoryItem.query.filter_by(school_id=current_user.school_id).all()
    categories = InventoryCategory.query.filter_by(school_id=current_user.school_id).all()
