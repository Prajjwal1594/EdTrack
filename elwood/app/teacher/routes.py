...
def announcements():
    ...
    return render_template('teacher/announcements.html', announcements=ann_list)
...
def my_attendance():
    ...
    if not staff:
        # Auto-create profile if missing for teacher
        staff = StaffProfile(user_id=current_user.id, school_id=current_user.school_id)
    ...
    return render_template('teacher/my_attendance.html', records=attendance_records, stats=stats)
