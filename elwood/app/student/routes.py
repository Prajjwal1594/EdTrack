...
from app.models import (Student, Grade, Attendance, Assignment, AssignmentSubmission,
                         ...
                         MicroCredential, TeacherAssignment, Subject, User, FeePayment,
...
def teachers():
    ...
    # Get teachers assigned to the student's section
    teacher_assignments = (TeacherAssignment.query
                          .filter_by(section_id=student.section_id)
                          .all())
    return render_template('student/teachers.html', student=student,
...
def announcements(atype=None):
    student = get_current_student()
    query = Announcement.query.filter_by(school_id=current_user.school_id, is_active=True)
...
def events():
    student = get_current_student()
    events_list = (Event.query.filter_by(school_id=current_user.school_id, is_active=True)
...
def summer_term():
    ...
    terms = (AcademicTerm.query.filter_by(school_id=current_user.school_id)
