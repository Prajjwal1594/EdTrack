...
@hr_required
def dashboard():
    staff_count = User.query.filter_by(school_id=current_user.school_id, role='teacher').count()
    pending_payroll = PayrollTransaction.query.filter_by(school_id=current_user.school_id, status='Pending').count()
...
def directory():
    ...
    # Ensure StaffProfile exists for all teachers
    for t in teachers:
        if not t.staff_profile:
            profile = StaffProfile(user_id=t.id, school_id=t.school_id)
            db.session.add(profile)
    db.session.commit()
    
    staff = StaffProfile.query.join(User).filter(User.school_id == current_user.school_id, User.role == 'teacher').all()
...
def attendance():
    ...
    if request.method == 'POST':
        for key, value in request.form.items():
            if key.startswith('status_'):
                ...
                # Verify this staff member is a teacher
                staff_check = StaffProfile.query.join(User).filter(StaffProfile.id == staff_id, User.role == 'teacher').first()
                ...
                if existing:
                    ...
                    new_att = StaffAttendance(
                        staff_id=staff_id, school_id=current_user.school_id,
    ...
    # Ensure StaffProfile exists for all teachers before listing
    teachers = User.query.filter_by(school_id=current_user.school_id, role='teacher').all()
    for t in teachers:
        if not t.staff_profile:
            profile = StaffProfile(user_id=t.id, school_id=t.school_id)
    ...
    staff = StaffProfile.query.join(User).filter(User.school_id == current_user.school_id, User.role == 'teacher').all()
    attendance_records = {a.staff_id: a.status for a in StaffAttendance.query.filter_by(school_id=current_user.school_id, date=selected_date_obj).all()}
...
def payroll():
    ...
    if request.method == 'POST' and request.form.get('action') == 'generate':
        ...
        existing = PayrollTransaction.query.filter_by(school_id=current_user.school_id, month=month, year=year).first()
        if existing:
            flash(f'Payroll for {month}/{year} has already been generated.', 'warning')
            return redirect(url_for('hr.payroll'))
            
        staff_list = StaffProfile.query.join(User).filter(User.school_id == current_user.school_id, User.role == 'teacher').all()
        for staff in staff_list:
            ...
            new_payroll = PayrollTransaction(
                staff_id=staff.id, school_id=current_user.school_id,
    ...
    transactions = PayrollTransaction.query.filter_by(school_id=current_user.school_id).order_by(PayrollTransaction.year.desc(), PayrollTransaction.month.desc()).all()
